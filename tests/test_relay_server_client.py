...
def test_relay_transfer_key_round_trip():
    """Relay keys encode a server URL and short code."""
...
def test_relay_fallback_downloads_archive_when_direct_fails(tmp_path):
    ...
    try:
        ...
        sender.close()
        session = create_relay_session(_server_url(server), pairing_key, archive)

        received_path = receive_with_relay_fallback(session.transfer_key, str(tmp_path / "in"))
...
def _start_relay_server(tmp_path):
    server = create_server("127.0.0.1", 0, str(tmp_path / "relay"), ttl_seconds=60)
